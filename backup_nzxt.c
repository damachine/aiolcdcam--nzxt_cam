...
#include <unistd.h>
// nzxt.c
// C-Version von nzxt.py: Liest CPU/GPU-Temperatur aus und erzeugt ein PNG-Bild
// Kompilieren mit: gcc nzxt.c -o nzxt $(pkg-config --cflags --libs cairo) -lcurl -lm
...
// Funktionsdeklarationen für CoolerControl
void init_sensor_paths();
...
// Farbverlauf für Temperaturbalken (grün → orange → rot)
void lerp_temp_color(float val, int* r, int* g, int* b) {
    if (val <= 55) { *r = 0; *g = 255; *b = 0; }
    else if (val <= 65) { *r = 255; *g = 140; *b = 0; }
    else if (val <= 75) { *r = 255; *g = 70; *b = 0; }
    else { *r = 255; *g = 0; *b = 0; }
}
 ...
 */
void get_cpu_stat(long *total, long *idle) {
    FILE *fstat = fopen("/proc/stat", "r");
    if (!fstat) { *total = 0; *idle = 0; return; }
    char line[256];
    long user = 0, nice = 0, system = 0, idle_val = 0, iowait = 0, irq = 0, softirq = 0, steal = 0;
    if (!fgets(line, sizeof(line), fstat)) { fclose(fstat); *total = 0; *idle = 0; return; }
    fclose(fstat);
    sscanf(line, "cpu  %ld %ld %ld %ld %ld %ld %ld %ld", &user, &nice, &system, &idle_val, &iowait, &irq, &softirq, &steal);
 ...
 * @return CPU-Auslastung in Prozent (float)
 */
float get_cpu_usage(long last_total, long last_idle, long curr_total, long curr_idle) {
    long totald = curr_total - last_total;
    long idled = curr_idle - last_idle;
    if (totald > 0) return 100.0f * (float)(totald - idled) / (float)totald;
    return 0.0f;
}
...
void draw_combined_image(const char *mode) {
    ...
    if (strcmp(mode, "def") != 0) {
        ...
        get_cpu_stat(&curr_total, &curr_idle);
        if (last_total != 0 && last_idle != 0) {
            cpu_usage = get_cpu_usage(last_total, last_idle, curr_total, curr_idle);
        }
        last_total = curr_total;
        last_idle = curr_idle;

        // RAM-Auslastung berechnen (nur für Modi 1, 2, 3)
        ...
        if (mem_total > 0 && mem_used >= 0) {
            ram_usage = 100.0f * (float)mem_used / (float)mem_total;
        }
    }
    ...
    // GPU-Daten je nach Modus optimiert abrufen
    if (strcmp(mode, "def") == 0) {
    ...
    // Layout-Konstanten
    int W = 240, H = 240;
    int box_w = 70, box_h = 60, box_gap = 10;
    int bar_w = 220, bar_h = 30, bar_gap = 6;
    int bar_x = (W - bar_w) / 2;
    int cpu_bar_y = (H - (2 * bar_h + bar_gap)) / 2;
    int gpu_bar_y = cpu_bar_y + bar_h + bar_gap;
    ...
    int cpu_box_x = 2 + box_w + box_gap;                  // X-Position CPU-Box (mittig oben)
    int cpu_box_y = 2;                                     // Y-Position CPU-Box (oberer Rand)
    int coolant_box_x = 2 + 2 * (box_w + box_gap);        // X-Position Coolant-Box (rechts oben)
    int coolant_box_y = 2;                                 // Y-Position Coolant-Box (oberer Rand)
    int gpu_box_x = 2 + box_w + box_gap;                  // X-Position GPU-Box (mittig unten)
    int gpu_box_y = H - box_h - 2;                        // Y-Position GPU-Box (unterer Rand)

    // --- CPU-Balken (mittig, ohne Text, mit Rand) ---
    int r, g, b;
    lerp_temp_color(cpu_temp, &r, &g, &b);
    int cpu_val_w = (int)(((cpu_temp - 0) / 100.0) * bar_w);
    if (cpu_val_w < 0) cpu_val_w = 0;
    if (cpu_val_w > bar_w) cpu_val_w = bar_w;
    cairo_set_source_rgb(cr, 0.16, 0.16, 0.16);
    cairo_rectangle(cr, bar_x, cpu_bar_y, bar_w, bar_h);
    cairo_fill(cr);
    cairo_set_source_rgb(cr, r/255.0, g/255.0, b/255.0);
    cairo_rectangle(cr, bar_x, cpu_bar_y, cpu_val_w, bar_h);
    cairo_fill(cr);
    ...
    int gpu_val_w = (int)(((gpu_temp - 0) / 100.0) * bar_w);
    if (gpu_val_w < 0) gpu_val_w = 0;
    if (gpu_val_w > bar_w) gpu_val_w = bar_w;
    cairo_set_source_rgb(cr, 0.16, 0.16, 0.16);
    cairo_rectangle(cr, bar_x, gpu_bar_y, bar_w, bar_h);
    cairo_fill(cr);
    cairo_set_source_rgb(cr, r/255.0, g/255.0, b/255.0);
    cairo_rectangle(cr, bar_x, gpu_bar_y, gpu_val_w, bar_h);
    cairo_fill(cr);
    cairo_set_line_width(cr, 2.0);
    cairo_set_source_rgb(cr, 0.08, 0.08, 0.08);
    cairo_rectangle(cr, bar_x, gpu_bar_y, bar_w, bar_h);
    ...
    if (strcmp(mode, "1") == 0) {
        // --- Vertikale Balken oben links (CPU/RAM) ---
        int bar_x_cpu = 10;
        int bar_y_cpu = 10;
        int bar_w_cpu = 6;
        int bar_h_cpu = box_h - 1;
        int bar_fill_cpu = (int)(bar_h_cpu * cpu_usage / 100.0f);
        cairo_set_source_rgb(cr, 0.3, 0.7, 1.0);
        cairo_rectangle(cr, bar_x_cpu, bar_y_cpu + bar_h_cpu - bar_fill_cpu, bar_w_cpu, bar_fill_cpu);
        cairo_fill(cr);
        int bar_x_ram = bar_x_cpu + bar_w_cpu + 6;
        int bar_y_ram = bar_y_cpu;
        int bar_w_ram = 6;
        int bar_h_ram = bar_h_cpu;
        int bar_fill_ram = (int)(bar_h_ram * ram_usage / 100.0f);
        cairo_set_source_rgb(cr, 0.8, 0.5, 1.0);
        cairo_rectangle(cr, bar_x_ram, bar_y_ram + bar_h_ram - bar_fill_ram, bar_w_ram, bar_fill_ram);
        cairo_fill(cr);

        // --- Vertikale Balken unten links (GPU/GPU-RAM) ---
        int bar_x_gpu = 10;
        int bar_y_gpu = H - bar_h_cpu - 10;
        int bar_w_gpu = 6;
        int bar_h_gpu = bar_h_cpu;
        int bar_fill_gpu = (int)(bar_h_gpu * gpu_usage / 100.0f);
        cairo_set_source_rgb(cr, 0.4, 1.0, 0.4);
        cairo_rectangle(cr, bar_x_gpu, bar_y_gpu + bar_h_gpu - bar_fill_gpu, bar_w_gpu, bar_fill_gpu);
        cairo_fill(cr);
        int bar_x_gpu_ram = bar_x_gpu + bar_w_gpu + 6;
        int bar_y_gpu_ram = bar_y_gpu;
        int bar_w_gpu_ram = 6;
        int bar_h_gpu_ram = bar_h_gpu;
        int bar_fill_gpu_ram = (int)(bar_h_gpu_ram * gpu_ram_usage / 100.0f);
        cairo_set_source_rgb(cr, 0.8, 0.5, 1.0);
        cairo_rectangle(cr, bar_x_gpu_ram, bar_y_gpu_ram + bar_h_gpu_ram - bar_fill_gpu_ram, bar_w_gpu_ram, bar_fill_gpu_ram);
        cairo_fill(cr);
    }
    // --- Zwei Kreisdiagramme für Mode 2 ---
    if (strcmp(mode, "2") == 0) {
        // Oben links: CPU-Auslastung (blau) und RAM-Auslastung (lila)
        double cx_top = 26; // Mehr Abstand vom linken Rand
        double cy_top = 26; // Mehr Abstand vom oberen Rand
        double radius_top = 22; // KLEINERER RADIUS
        double thickness = 6;   // Dünnere Segmente
        
        // Hintergrundkreise (grau) für bessere Sichtbarkeit
        cairo_set_line_width(cr, thickness);
        cairo_set_source_rgb(cr, 0.2, 0.2, 0.2);
        cairo_arc(cr, cx_top, cy_top, radius_top, 0, 2 * M_PI);
        cairo_stroke(cr);
        cairo_arc(cr, cx_top, cy_top, radius_top - thickness - 2, 0, 2 * M_PI);
        cairo_stroke(cr);
        
        // CPU-Auslastung (blau)
        double cpu_angle = 2 * M_PI * cpu_usage / 100.0;
        cairo_set_line_width(cr, thickness);
        cairo_set_source_rgb(cr, 0.3, 0.7, 1.0);
        cairo_arc(cr, cx_top, cy_top, radius_top, -M_PI/2, -M_PI/2 + cpu_angle);
        cairo_stroke(cr);
        
        // RAM-Auslastung (lila, innerer Ring)
        double ram_angle = 2 * M_PI * ram_usage / 100.0;
        cairo_set_line_width(cr, thickness);
        cairo_set_source_rgb(cr, 0.8, 0.5, 1.0);
        cairo_arc(cr, cx_top, cy_top, radius_top - thickness - 2, -M_PI/2, -M_PI/2 + ram_angle);
        cairo_stroke(cr);

        // Unten links: GPU-Auslastung (grün) und GPU-RAM-Auslastung (lila)
        double cx_bot = 26; // Mehr Abstand vom linken Rand
        double cy_bot = H - 26; // Mehr Abstand vom unteren Rand
        double radius_bot = 22; // KLEINERER RADIUS
        
        // Hintergrundkreise (grau) für bessere Sichtbarkeit
        cairo_set_line_width(cr, thickness);
        cairo_set_source_rgb(cr, 0.2, 0.2, 0.2);
        cairo_arc(cr, cx_bot, cy_bot, radius_bot, 0, 2 * M_PI);
        cairo_stroke(cr);
        cairo_arc(cr, cx_bot, cy_bot, radius_bot - thickness - 2, 0, 2 * M_PI);
        cairo_stroke(cr);
        
        // GPU-Auslastung (grün)
        double gpu_angle = 2 * M_PI * gpu_usage / 100.0;
        cairo_set_line_width(cr, thickness);
        cairo_set_source_rgb(cr, 0.4, 1.0, 0.4);
        cairo_arc(cr, cx_bot, cy_bot, radius_bot, -M_PI/2, -M_PI/2 + gpu_angle);
        cairo_stroke(cr);
        
        // GPU-RAM-Auslastung (lila, innerer Ring)
        double gpu_ram_angle = 2 * M_PI * gpu_ram_usage / 100.0;
        cairo_set_line_width(cr, thickness);
        cairo_set_source_rgb(cr, 0.8, 0.5, 1.0);
        cairo_arc(cr, cx_bot, cy_bot, radius_bot - thickness - 2, -M_PI/2, -M_PI/2 + gpu_ram_angle);
        cairo_stroke(cr);
    }
    // --- Horizontale Auslastungsbalken für Mode 3 ---
    if (strcmp(mode, "3") == 0) {
        // CPU-Auslastung (blau)
        int cpu_usage_bar_x = bar_x + 1;
        int cpu_usage_bar_y = cpu_bar_y + 1;
        int cpu_usage_bar_w = (int)(bar_w * cpu_usage / 100.0f) - 2;
        int cpu_usage_bar_h = 5;
        cairo_set_source_rgb(cr, 0.3, 0.7, 1.0);
        cairo_rectangle(cr, cpu_usage_bar_x, cpu_usage_bar_y, cpu_usage_bar_w, cpu_usage_bar_h);
        cairo_fill(cr);
        // RAM-Auslastung (lila)
        int ram_usage_bar_x = bar_x + 1;
        int ram_usage_bar_y = cpu_usage_bar_y + 6;
        int ram_usage_bar_w = (int)(bar_w * ram_usage / 100.0f) - 2;
        int ram_usage_bar_h = 5;
        cairo_set_source_rgb(cr, 0.8, 0.5, 1.0);
        cairo_rectangle(cr, ram_usage_bar_x, ram_usage_bar_y, ram_usage_bar_w, ram_usage_bar_h);
        cairo_fill(cr);
        // GPU-Auslastung (gelb)
        int gpu_usage_bar_x = bar_x + 1;
        int gpu_usage_bar_y = gpu_bar_y + 1;
        int gpu_usage_bar_w = (int)(bar_w * gpu_usage / 100.0f) - 2;
        int gpu_usage_bar_h = 5;
        cairo_set_source_rgb(cr, 1.0, 1.0, 0.0);
        cairo_rectangle(cr, gpu_usage_bar_x, gpu_usage_bar_y, gpu_usage_bar_w, gpu_usage_bar_h);
        cairo_fill(cr);
        // GPU-RAM-Auslastung (lila)
        int gpu_ram_usage_bar_x = bar_x + 1;
        int gpu_ram_usage_bar_y = gpu_bar_y + 6;
        int gpu_ram_usage_bar_w = (int)(bar_w * gpu_ram_usage / 100.0f) - 2;
        int gpu_ram_usage_bar_h = 5;
        cairo_set_source_rgb(cr, 0.8, 0.5, 1.0);
        cairo_rectangle(cr, gpu_ram_usage_bar_x, gpu_ram_usage_bar_y, gpu_ram_usage_bar_w, gpu_ram_usage_bar_h);
        cairo_fill(cr);
    }

    // Schriftart für Temperaturwerte
    cairo_select_font_face(cr, "DejaVuSans-Bold", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD);

    // Layout- und Textvariablen für Temperaturanzeigen
    char temp_str[8];
    char cool_str[16];
    double temp_x, temp_y, cool_x, cool_y;
    double font_size = box_h * 1.50;
    double deg_font_size = font_size * 0.75;
    double coolant_font_size = font_size * 0.30;
    ...
    temp_x = cpu_box_x + (box_w - ext.width) / 2;
    temp_y = cpu_box_y + ext.height + 8;
    cairo_set_source_rgb(cr, 1, 1, 1);
    ...
    cairo_move_to(cr, temp_x + ext.width + 8, temp_y - deg_ext.height + 4);
    cairo_show_text(cr, "°");
    ...
    if (cpu_temp <= 99.0f) {
        ...
        cool_x = coolant_box_x + (box_w - cool_ext.width) / 2;
        cool_y = coolant_box_y + box_h + 8;
        cairo_set_source_rgb(cr, 1, 1, 1);
    ...
    cairo_set_font_size(cr, font_size);
    cairo_text_extents(cr, temp_str, &ext);
    temp_x = gpu_box_x + (box_w - ext.width) / 2;
    temp_y = gpu_box_y + box_h - 8;
    cairo_set_source_rgb(cr, 1, 1, 1);
    cairo_move_to(cr, temp_x, temp_y);
    cairo_show_text(cr, temp_str);
    cairo_set_font_size(cr, deg_font_size);
    cairo_text_extents(cr, "°", &deg_ext_gpu);
    cairo_move_to(cr, temp_x + ext.width + 8, temp_y - deg_ext_gpu.height + 4);
    cairo_show_text(cr, "°");

    static float last_cpu_temp = -1, last_gpu_temp = -1, last_coolant_temp = -1;
    static float last_cpu_usage = -1, last_gpu_usage = -1, last_ram_usage = -1, last_gpu_ram_usage = -1;
    ...
    if (fabs(cpu_temp - last_cpu_temp) > TEMP_TOLERANCE ||
        fabs(gpu_temp - last_gpu_temp) > TEMP_TOLERANCE ||
        fabs(coolant_temp - last_coolant_temp) > TEMP_TOLERANCE ||
        fabs(cpu_usage - last_cpu_usage) > USAGE_TOLERANCE ||
        fabs(gpu_usage - last_gpu_usage) > USAGE_TOLERANCE ||
        fabs(ram_usage - last_ram_usage) > USAGE_TOLERANCE ||
        fabs(gpu_ram_usage - last_gpu_ram_usage) > USAGE_TOLERANCE ||
        !did_mkdir) {
        needs_update = 1;
    }
    ...
    if (needs_update) {
        ...
        if (cairo_surface_write_to_png(surface, IMAGE_PATH) == CAIRO_STATUS_SUCCESS) {
            ...
            // Bild 2x an das Display senden (wie im ursprünglichen Code)
